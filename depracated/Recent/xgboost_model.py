"""XGBoost training script extracted from the notebook workflow.
...
def evaluate_model_cv(
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), start=1):
        ...
        fold_model.fit(X_train_proc, y_train, verbose=False)
...
def print_hyperparameter_search_table() -> None:
    ...
    print("\n" + "=" * 70)
    print("TABEL RENTANG HYPERPARAMETER YANG DIUJI (XGBoost)")
    print("=" * 70)
...
def run_hyperparameter_tuning(X: pd.DataFrame, y: pd.Series) -> dict:
    ...
    print(f"Best CV F1 XGB  : {grid.best_score_:.4f}")

    return best_params


def run_label_shuffle_sanity_test(X: pd.DataFrame, y: pd.Series, model: XGBClassifier) -> None:
...
def main() -> None:
    ...
    xgb_params_repr = get_representative_params(result["best_params_per_fold"])
    print(f"\nHyperparameter representatif (modus): {xgb_params_repr}")

    # OOF classification report
    print("\nClassification Report - XGBoost (Out-of-Fold):")
    print(
        classification_report(
            result["oof_y_true"],
            result["oof_y_pred"],
    ...
    base_model = XGBClassifier(**xgb_params_repr)
